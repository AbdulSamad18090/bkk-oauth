...
app.use(cors({
    origin: function (origin, callback) {
        if ([undefined, null, 'null', 'undefined'].includes(origin)) return callback(null, true);
...
server.grant(oauth2orize.grant.code((client, redirectUri, user, ares, done) => {
    ...
    models.authCodes.push({ code, clientId: client.clientId, redirectUri, userId: user.id });
    done(null, code);
}));
...
server.exchange(oauth2orize.exchange.password(async (client, username, password, scope, done) => {
    ...
    models.tokens.push({ accessToken, refreshToken, userId: user.id, clientId: client.clientId });
...
server.exchange(oauth2orize.exchange.code((client, code, redirectUri, done) => {
    ...
    const authCode = models.authCodes.find(c => c.code === code && c.clientId === client.clientId);
    if (!authCode || authCode.redirectUri !== redirectUri) return done(null, false);

    const accessToken = crypto.randomBytes(32).toString("hex");
    const refreshToken = crypto.randomBytes(32).toString("hex");

    models.tokens.push({ accessToken, refreshToken, userId: authCode.userId, clientId: authCode.clientId });
...
server.exchange(oauth2orize.exchange.refreshToken(async (client, refreshToken, done) => {
    const token = models.tokens.find(t => t.refreshToken === refreshToken);

    console.log("server.exchange.refreshToken ", { client, refreshToken, token });
    ...
    models.tokens.push({ accessToken: newAccessToken, refreshToken: newRefreshToken, userId: token.userId, clientId: token.clientId });
    return done(null, newAccessToken, newRefreshToken, { expires_in: 3600 });
}));
...
app.get("/authorize",
    ...
    server.authorize((clientId, redirectUri, done) => {
        ...
        return (client && client.redirectUri === redirectUri) ? done(null, client, redirectUri) : done(null, false);
    }),
    async (req, res) => {
...
app.get("/user", logRequest, passport.authenticate("bearer", { session: false }), (req, res, next) => {
    res.send(req.user);
});

app.get("/client", logRequest, passport.authenticate("bearer", { session: false }), (req, res, next) => {
...
app.post('/auth/saml/callback', passport.authenticate('saml', {
  ...
  failureFlash: true,
    ...
    const code = crypto.randomBytes(16).toString("hex");
    models.authCodes.push({ code, clientId: 'test', redirectUri: 'http://localhost:4113/dev-portal/', userId: req.user.id });
    const url = 'http://localhost:4113/dev-portal/authorize-login?code='+ code
    console.log({url})
    res.redirect(url);
});
